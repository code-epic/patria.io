...
function activarActualizar() {
    ...
    var prv = JSON.parse(atob(sessionStorage.getItem("patria.IO").split(".")[1]));
...
function EnviarArchivosIMG() {
    ...
    var strUrl = Conn.URLSEC + "/ipsfa/api/militar/jwtsubirarchivosx/" + ObjMilitar.Componente.abreviatura.toLowerCase();
    ...
    $.ajax({
        ...
        beforeSend: function (xhr) {
            xhr.setRequestHeader("Authorization", 'Bearer ' + sessionStorage.getItem('patria.IO'));
        }
    })
        .done(function (res) {
            $("#archivo").val("");
